        ...
        for EntryId in selectRowsEntryIds:
            ...
            if SQLRows.Count>0:
                if SQLRows[0]["FCONFIRMSTATUS"]=="A":
                    this.View.ShowMessage(SQLRows[0]["FBillNo"]+" 单据未确认，请确认后再生成。")
                    return
        
                if SQLRows[0]["FENTRYSTATUS"]=="B":
                    Message=SQLRows[0]["FBillNo"]+" 第"+str(SQLRows[0]["FSeq"])+"行不满足生成条件：行状态=未关闭"
                    return
                ...
                if PushResult.find("失败")>-1:
                    this.View.ShowMessage(SQLRows[0]["FBillNo"]+" 生成失败："+PushResult)
                    return
        this.View.ShowMessage("生成操作完成。")
